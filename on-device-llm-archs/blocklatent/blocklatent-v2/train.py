...
if config.iteration_init:
    model.ar_decoder.iteration_init()
...
for step in range(init_step + 1, num_steps + 1):
        ...
        with torch.no_grad():
            with amp_ctx:
                for _ in range(val_steps):
                    ...
                    _, output_info, _, _ = model(token_val, config)
    ...
    with profile_ctx as prof:
        if config.lm_head_c_mode:
            ...
            lm_head_loss = (
                vec_label_loss + lm_head_aux_loss
            ) * config.accumulated_steps
            if config.scaler:
        ...
        for micro_step in range(accumulated_steps):
            ...
            with sync_ctx:
                with amp_ctx:
                    ...
                    loss, step_output_info, logit_latent, target_latent = model(tokens, config)
                    ...
                    if step % 64 == 0 and micro_step < 2:
                        data_msg = f"token: {tokens[0, 11:111]}"
                        logit_msg = f"logit: {logit_latent[:200].max(-1)[1]}"
                        target_msg = f"target: {target_latent[:200].max(-1)[1]}"
                        target_msg_1 = f"{target_latent.topk(10, -1)[0].mean(0)}"
                        target_msg_2 = f"{target_latent.mean(0).topk(10)[0]}"
                        logger.log(logging.INFO, data_msg)
                        logger.log(logging.INFO, logit_msg)
                        logger.log(logging.INFO, target_msg)
                        logger.log(logging.INFO, target_msg_1)
                        logger.log(logging.INFO, target_msg_2)
                        # TODO: logit_latent dist, logit == target
    ...
    if master_process:
        ...
        main_loss, latent_loss, aux_loss, y_vec_loss, step_real_token_num = output_info
