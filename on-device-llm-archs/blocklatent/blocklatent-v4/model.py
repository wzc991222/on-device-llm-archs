...
class MainModel(nn.Module):
    ...
    def forward(
        ...
        def mask_mod(b, h, q_idx, kv_idx):
            ...
            causal_mask = q_b_idx >= kv_b_idx
            doc_mask = doc[b, q_b_idx] == doc[b, kv_b_idx]
            if config.main_attn_type == 0:
                return causal_mask & doc_mask
            else:
                q_r_idx = q_idx % e
                kv_r_idx = kv_idx % e
                sub_mask = q_r_idx == kv_r_idx
                if config.main_attn_type == 1:
                    return causal_mask & doc_mask & sub_mask
                if config.main_attn_type == 2:
                    block_mask = q_b_idx == kv_b_idx
                    return causal_mask & doc_mask & sub_mask | block_mask
...
class LMHead(nn.Module):
    ...
    def update(self, config: Config):
        ...
        if config.lm_head_aux_loss_factor != 0:
            num = self.lm_head_c_labels.numel()
            c_num = config.lm_head_c_num
            c_freqs = torch.bincount(self.lm_head_c_labels, minlength=c_num)
            c_freqs = c_freqs / num
            c_probs = vec_probs.sum(0) / num
            lm_head_aux_loss = (
                torch.dot(c_freqs, c_probs) * c_num * config.lm_head_aux_loss_factor
            )
    ...
    def forward(
        ...
        if config.lm_head_c_mode and config.y_vec_loss_factor != 0:
            ...
            vec_probs = self.probs_compute(vec, config)
            y_vec_loss = -torch.sum(vec_probs.detach() * torch.log(y_probs + 1e-10), -1)
            y_vec_loss = (y_vec_loss * mask).sum() / mask_sum * config.y_vec_loss_factor

        return main_loss, y_vec_loss, logits.detach(), labels
...
class Model(nn.Module):
    def __init__(self, config: Config):
        ...
        self.ar_vocab_emb = nn.Embedding(config.vocab_size, config.main_dim)
        self.codebook = nn.Parameter(torch.randn(config.main_dim, config.codebook_size))
        if config.sole_latent_vocab:
            self.sole_codebook = nn.Parameter(
                torch.randn(config.main_dim, config.codebook_size)
            )
        ...
        self.pre_lm_head = nn.Linear(config.main_dim, config.vocab_size, bias=False)
        self.lm_head = LMHead(config)
        self.x_latent_norm = nn.RMSNorm(config.main_dim, eps=config.norm_eps)
        ...
        if config.pre_tied_vocab_emb:
            self.vocab_emb.weight = self.pre_lm_head.weight
        if config.tied_vocab_emb:
            self.ar_vocab_emb.weight = self.lm_head.lm_head.weight
        self.init_weights(config)

    def main_mode_init(self, config: Config):
        freeze_module_list = [
            self.vocab_emb,
            self.encoder,
            self.aux_decoder,
        ...
        if config.sole_latent_vocab and config.copy_latent_vocab:
            with torch.no_grad():
                self.sole_codebook.data = self.codebook.data
    ...
    def pre_model(
        ...
        tokens = tokens[:, 1:]
        b, s = tokens.shape
        input = rearrange(tokens, "b (l n) -> b l n", n=n).clone()
        eot = (input == config.eot_idx).any(-1)
        doc = torch.zeros_like(eot)
        doc[:, 1:] = eot.cumsum(-1)[:, :-1]
        pad_num = (input == config.pad_idx).sum(-1)
        if config.encoder_mask:
            mask_prob = torch.full(input.shape, config.mask_rate, device=device)
            mask_prob.masked_fill_(input == config.pad_idx, value=0.0)
            mask = torch.bernoulli(mask_prob).bool()
            input[mask] = config.mask_idx
        ...
        if config.latent_vq:
            x_latent = x_latent.flatten(0, -2)
            x_latent_norm = self.x_latent_norm(x_latent)
            latent_logits = x_latent_norm @ self.codebook
            latent_labels = torch.max(latent_logits, -1)[1]
            vq = self.codebook.t()[latent_labels]
            vq_prob = torch.full(x_latent.shape[:-1], config.vq_rate, device=device)
            vq_mask = torch.bernoulli(vq_prob).bool()
            vq_mask = repeat(vq_mask, "bs -> bs d", d=d)
            if config.vq_st:
                x_latent = x_latent + torch.where(vq_mask, (vq - x_latent).detach(), 0)
            else:
                x_latent = torch.where(vq_mask, vq, x_latent)
            x_latent = rearrange(x_latent, "(b s) d -> b s d", b=b)

            token_num = latent_labels.numel()
            freq = torch.bincount(latent_labels, minlength=config.codebook_size)
            latent_probs = latent_logits.softmax(-1)
            freq_probs = freq / token_num
            probs = latent_probs.sum(0) / token_num
            aux_loss = (
                torch.dot(freq_probs, probs)
                * config.codebook_size
                * config.aux_loss_factor
            )

            if config.codebook_loss:
                if config.codebook_loss_type == "cross_entropy":
                    threshold = config.codebook_threshold
                    if threshold > 0:
                        probs_max, probs_idx = latent_probs.max(-1)
                        indices = probs_max < threshold
                        s_latent_logits = latent_logits[indices]
                        if config.codebook_label_prob_mode:
                            s_num = s_latent_logits.shape[0]
                            labels_idx = torch.arange(s_num)
                            probs_idx = probs_idx[indices]
                            s_latent_labels = latent_probs[indices].detach()
                            if config.codebook_label_prob_norm:
                                probs_max = probs_max[indices]
                                factor = (1 - threshold) / (1 - probs_max.detach())
                                s_latent_labels *= factor.unsqueeze(-1)
                                s_latent_labels[labels_idx, probs_idx] = threshold
                            else:
                                s_latent_labels[labels_idx, probs_idx] = threshold
                        else:
                            s_latent_labels = latent_labels[indices]
                    else:
                        s_latent_logits = latent_logits
                        s_latent_labels = latent_labels
                    codebook_loss = (
                        F.cross_entropy(
                            s_latent_logits,
                            s_latent_labels,
                ...
                if config.codebook_loss_type == "mse":
                    x_latent_norm = F.normalize(x_latent.flatten(0, -2), dim=-1) * sd
                    vq_norm = F.normalize(vq, dim=-1) * sd
                    codebook_loss = (
                        F.mse_loss(x_latent_norm, vq_norm, reduction="mean")
                        * config.codebook_loss_factor
                    )

        if config.orthog_loss:
            x_l = rearrange(x_latent, "b (l n) d -> (b l) n d", n=n)
            x_l = F.normalize(x_l, dim=-1)
            x_lt = x_l.transpose(1, 2)
            eye = torch.eye(n, device=x_l.device).unsqueeze(0)
            score = (torch.bmm(x_l, x_lt) - eye) ** 2
            orthog_loss = score.mean() * config.orthog_loss_factor

        y = self.aux_decoder(x_latent, doc, config)
        logits = self.pre_lm_head(y).flatten(0, -2)
        labels = tokens.flatten()
        weight = (labels != config.pad_idx) * config.unmask_factor
        if config.encoder_mask:
            weight = weight + mask.flatten() * (1 - config.unmask_factor)
        mask_loss = F.cross_entropy(logits, labels, reduction="none")
        mask_loss = (mask_loss * weight).sum() / weight.sum()
        pre_loss = mask_loss + aux_loss + codebook_loss + orthog_loss

        real_token_num = b * s - pad_num.sum()
        pre_info = torch.tensor(
            [
                mask_loss,
                aux_loss,
                codebook_loss,
        ...
        latent_logits_info = None
        x_latent_info = None
        vq_info = None
        if config.latent_vq:
            if config.codebook_loss_type == "mse":
                x_latent_info = x_latent_norm.detach()
                vq_info = vq_norm.detach()
            else:
                latent_logits_info = latent_logits.detach()
    ...
    def forward(
        ...
        begin[:, 1] = True
        pad_num = (input == config.pad_idx).sum(-1)
        if config.encoder_mask and config.main_encoder_mask:
            mask_prob = torch.full(input.shape, config.mask_rate, device=device)
            mask_prob.masked_fill_(input == config.pad_idx, value=0.0)
            mask = torch.bernoulli(mask_prob).bool()
            input[mask] = config.mask_idx
        x = self.vocab_emb(input)
        x_ar = self.ar_vocab_emb(input_ar)
        x_latent = self.encoder(x, doc, config)
        ...
        if config.latent_loss:
            ...
            if config.latent_loss_type == "cross_entropy":
                y_latent = self.y_latent_norm(y_latent)
                label_latent = self.x_latent_norm(label_latent)
                if config.sole_latent_vocab:
                    y_logit = y_latent @ self.sole_codebook
                else:
                    y_logit = y_latent @ self.codebook
                label_prob = (label_latent @ self.codebook).softmax(-1).detach()
                latent_loss = F.cross_entropy(y_logit, label_prob, reduction="none")

            if config.latent_loss_type == "mse":
                y_latent = F.normalize(y_latent, dim=-1) * sd
                label_latent = F.normalize(label_latent, dim=-1) * sd
                latent_loss = F.mse_loss(y_latent, label_latent, reduction="none")
                latent_loss = latent_loss.mean(-1)

            weight = torch.where(eot, 0, (n - pad_num) / n)
            weight = repeat(weight[:, :-1], "b l -> (b l e)", e=ms)
        ...
        main_loss, y_vec_loss, logits, labels = self.lm_head(y, tokens, config)
        loss = main_loss + latent_loss + y_vec_loss
        ...
        if config.latent_loss:
            if config.latent_loss_type == "cross_entropy":
                y_info = y_logit.detach()
                label_info = label_prob
            if config.latent_loss_type == "mse":
                y_info = y_latent.detach()
                label_info = label_latent.detach()

        return loss, info, logits, labels, y_info, label_info
    ...
    def params_count(self, config: Config):
        ...
        infer_model_a_params = model_a_params - encoder_a_params - aux_decoder_a_params

        lm_head_a_params = 2 * d * v
        codebook_a_params = int(d * lv * ms / n)
        if config.lm_head_c_mode:
            lm_head_a_params += d * hn * vn
        latent_vocab_a_params = 0
        if config.cross_entropy_loss:
            latent_vocab_a_params = 2 * codebook_a_params
            if config.latent_vq:
                latent_vocab_a_params += codebook_a_params
        vocab_a_params = lm_head_a_params + latent_vocab_a_params
        infer_vocab_a_params = lm_head_a_params - d * v
        total_a_params = model_a_params + vocab_a_params
        infer_total_a_params = infer_model_a_params + infer_vocab_a_params
        ...
        activated_params_result = {
            ...
            "infer_model_a_params": infer_model_a_params,
            "infer_vocab_a_params": infer_vocab_a_params,
            "infer_total_a_params": infer_total_a_params,
        }
