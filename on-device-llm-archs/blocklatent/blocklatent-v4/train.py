...
pre_mode = config.train_mode == "pre"
main_mode = config.train_mode == "main"
m = config.main_size
n = config.block_size
...
if config.main_mode_init:
    model.main_mode_init(config)
...
for step in range(init_step + 1, num_steps + 1):
        ...
        with torch.no_grad():
            with amp_ctx:
                for _ in range(val_steps):
                    ...
                    if pre_mode:
                        info = model.pre_model(token_val, config)[1]
                    if main_mode:
                        info = model(token_val, config)[1]
                    val_loss += info[0] / val_steps
        if ddp:
    ...
    with profile_ctx as prof:
        if main_mode and config.lm_head_c_mode:
                ...
                with amp_ctx:
                    if pre_mode:
                        (
                            ...
                            x_latent_norm,
                            vq_norm,
                        ) = model.pre_model(tokens, config)
                        pre_loss = pre_loss / accumulated_steps
                        pre_info += step_pre_info / accumulated_steps
                        if config.scaler:
                    ...
                    if main_mode:
                        loss, step_info, logits, labels, y_info, label_info = model(
                            tokens, config
                        )
                        loss = loss / accumulated_steps
                        info += step_info / accumulated_steps
                        if config.scaler:
        ...
        if pre_mode and ddp:
            dist.all_reduce(pre_info, op=dist.ReduceOp.AVG)

        if main_mode and ddp:
            dist.all_reduce(info, op=dist.ReduceOp.AVG)

        if step % 50 == 0 and pre_mode and config.latent_vq:
            if config.codebook_loss_type == "mse":
                cos = (x_latent_norm * vq_norm).sum(-1).mean() / config.main_dim
                msg_1 = f"cos: {cos}"
            else:
                prob_topk = latent_logits.softmax(-1).topk(10, -1)[0].mean(0)
                msg_1 = f"latent prob topk: {prob_topk}"
            probs = logits.softmax(-1)
            probs_label = torch.gather(probs, 1, labels.unsqueeze(-1)).squeeze(-1)
            diff = ((1 - probs_label) * weight).sum() / weight.sum() * 2
            msg_2 = f"diff: {diff}"
            logger.log(logging.INFO, msg_1)
            logger.log(logging.INFO, msg_2)
            del (
                latent_logits,
                logits,
                probs,
        ...
        if step % 50 == 0 and main_mode:
            main_probs = logits.softmax(-1)
            main_probs_label = torch.gather(
                main_probs, 1, labels.unsqueeze(-1)
            ).squeeze(-1)
            main_diff = f"main diff: {(1 - main_probs_label).mean() * 2}"

            if config.latent_loss_type == "cross_entropy":
                y_info = y_info.softmax(-1)
                sample_size = 64 // m * m
                y_sample = y_info[:sample_size].max(-1)[1].reshape(-1, m)
                label_sample = label_info[:sample_size].max(-1)[1].reshape(-1, m)
                y_msg = f"y: {y_sample}"
                label_msg = f"label: {label_sample}"
                prob_diff = f"prob diff: {(y_info - label_info).abs().sum(-1).mean()}"
                msg_1 = f"y topk: {y_info.topk(10, -1)[0].mean(0)}"
                msg_2 = f"label topk: {label_info.topk(10, -1)[0].mean(0)}"
                msg_3 = f"main topk: {main_probs.topk(10, -1)[0].mean(0)}"
                ...
                logger.log(logging.INFO, msg_1)
                logger.log(logging.INFO, msg_2)
                logger.log(logging.INFO, msg_3)
                del y_sample, label_sample

            if config.latent_loss_type == "mse":
                cos = (y_info * label_info).sum(-1).mean() / config.main_dim
                cos_msg = f"cos: {cos}"
                label_msg = f"main topk: {main_probs.topk(10, -1)[0].mean(0)}"
                logger.log(logging.INFO, cos_msg)
                logger.log(logging.INFO, main_diff)
                logger.log(logging.INFO, label_msg)
