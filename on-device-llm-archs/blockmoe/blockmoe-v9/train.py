...
p_block_size = config.p_block_size
p_seq_block_num = config.p_seq_block_num
...
class DistributedDataLoader:
    def __init__(
        ...
        self.p_block_size = p_block_size
        self.p_seq_block_num = p_seq_block_num
...
for step in range(init_step + 1, num_steps + 1):
        ...
        with torch.no_grad():
            with amp_ctx:
                for i in range(val_steps):
                    ...
                    step_val_loss = model(x, y, config, False, False)[1]
    ...
    with profile_ctx as prof:
        ...
        for i in range(sum_steps):
            ...
            with torch.no_grad():
                with amp_ctx:
                    model(x, y, config, False, True)

        for i in range(sum_steps):
            x, y = x_list[i], y_list[i]
            sync_ctx = model.no_sync() if ddp and i < sum_steps - 1 else nullcontext()
            with sync_ctx:
                with amp_ctx:
                    loss, step_main_loss, step_main_pb_loss, step_main_fb_loss = model(
                        x, y, config, False, False
                    )
                    loss = loss / sum_steps
                    main_loss += step_main_loss / sum_steps
                    main_pb_loss += step_main_pb_loss / sum_steps
                    main_fb_loss += step_main_fb_loss / sum_steps
                    if config.scaler:
        ...
        p_count = aux_model.p_count
        f_count = aux_model.f_count

        if p_count > 0:
            avg_p_stat = aux_model.p_stat / p_count
            avg_p_logits_max = aux_model.p_logits_max / p_count
            avg_p_logits_mean = aux_model.p_logits_mean / p_count
            logger.log(logging.INFO, f"main p indices: {avg_p_stat}")
            logger.log(logging.INFO, f"main p indices max: {avg_p_stat.max()}")
            logger.log(logging.INFO, f"main p logits max: {avg_p_logits_max}")
            logger.log(logging.INFO, f"main p logits mean: {avg_p_logits_mean}")
            aux_model.p_reset()

        if f_count > 0:
            avg_f_stat = aux_model.f_stat / f_count
            avg_f_logits_max = aux_model.f_logits_max / f_count
            avg_f_logits_mean = aux_model.f_logits_mean / f_count
            logger.log(logging.INFO, f"main f indices: {avg_f_stat}")
            logger.log(logging.INFO, f"main f indices max: {avg_f_stat.max()}")
            logger.log(logging.INFO, f"main f logits max: {avg_f_logits_max}")
            logger.log(logging.INFO, f"main f logits mean: {avg_f_logits_mean}")
            aux_model.f_reset()

        for i in range(aux_sum_steps):
            aux_x = train_loader.next_aux_batch()
            aux_x = aux_x.to(device)
            sync_ctx = (
                model.no_sync() if ddp and i < aux_sum_steps - 1 else nullcontext()
            )
            with sync_ctx:
                with amp_ctx:
                    loss, pb_loss, fb_loss = model(aux_x, None, config, True, True)
                    aux_pb_loss += pb_loss / aux_sum_steps
                    aux_fb_loss += fb_loss / aux_sum_steps
                    if config.scaler:
                        scaler.scale(loss).backward()
                    else:
                        loss.backward()
        ...
        if aux_sum_steps > 0:
            ...
            avg_p_logits_max = aux_model.p_logits_max / p_count
            avg_p_logits_mean = aux_model.p_logits_mean / p_count
            logger.log(logging.INFO, f"aux p indices: {avg_p_stat}")
            logger.log(logging.INFO, f"aux p indices max: {avg_p_stat.max()}")
            logger.log(logging.INFO, f"aux p logits max: {avg_p_logits_max}")
            logger.log(logging.INFO, f"aux p logits mean: {avg_p_logits_mean}")
            aux_model.p_reset()
            ...
            avg_f_logits_max = aux_model.f_logits_max / f_count
            avg_f_logits_mean = aux_model.f_logits_mean / f_count
            logger.log(logging.INFO, f"aux f indices: {avg_f_stat}")
            logger.log(logging.INFO, f"aux f indices max: {avg_f_stat.max()}")
            logger.log(logging.INFO, f"aux f logits max: {avg_f_logits_max}")
            logger.log(logging.INFO, f"aux f logits mean: {avg_f_logits_mean}") 
            aux_model.f_reset()
        ...
        if ddp:
            ...
            dist.all_reduce(main_pb_loss, op=dist.ReduceOp.AVG)
            dist.all_reduce(main_fb_loss, op=dist.ReduceOp.AVG)
            dist.all_reduce(aux_pb_loss, op=dist.ReduceOp.AVG)
            dist.all_reduce(aux_fb_loss, op=dist.ReduceOp.AVG)
    ...
    if master_process:
        ...
        msg = (
            ...
            f"main_pb_loss: {main_pb_loss:.4f} "
            f"main_fb_loss: {main_fb_loss:.4f} "
            f"aux_pb_loss: {aux_pb_loss:.4f} "
            f"aux_fb_loss: {aux_fb_loss:.4f} "
        ...
        wandb_stat = {
            "main_loss": main_loss,
            "main_pb_loss": main_pb_loss,
            "main_fb_loss": main_fb_loss,
            "aux_pb_loss": aux_pb_loss,
            "aux_fb_loss": aux_fb_loss,
            "step_time": time_ms,
