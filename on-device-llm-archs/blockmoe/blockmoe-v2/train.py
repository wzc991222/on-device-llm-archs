...
class DistributedDataLoader:
    def __init__(
        ...
        self.num_token = batch_size * (seq_len + 1)
...
for step in range(init_step + 1, num_steps + 1):
    ...
    with profile_ctx as prof:
        for micro_step in range(accumulated_steps):
            ...
            with sync_ctx:
                with amp_ctx:
                    loss, step_info = model(tokens, config)
    ...
    if master_process:
        ...
        main_loss, aux_pred_loss, balance_loss, step_token_num = info
        token_num += step_token_num * accumulated_steps
        token_rate = token_num / ((step - init_step) * accumulated_step_tokens)

        msg = (
            f"\nstep: {step}/{num_steps} "
            f"main_loss: {main_loss:.4f} "
            f"aux_pred_loss: {aux_pred_loss:.3f} "
