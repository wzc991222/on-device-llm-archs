...
class AuxRouter(nn.Module):
    ...
    def forward(
        ...
        s = x.shape[1]
        mask_mod = mask_mod_2 if config.aux_global_attn else mask_mod_1
...
class AuxModel(nn.Module):
    ...
    def prepare(self, input: torch.LongTensor, config: Config):
        n = config.block_size
        rs = config.router_size
        eot = input == config.eot_idx
        eot = rearrange(eot, "b (l n) -> b l n", n=n)
        b, l = eot.shape[:2]
        router_pad = torch.zeros(1, 1, 1, device=eot.device).expand(b, l, rs)
        eot = torch.cat([eot, router_pad], -1).flatten(1)
    ...
    def forward(
        ...
        indices = logits_m.topk(gt, -1, sorted=False)[1]
        values = torch.gather(logits, -1, indices)

        indices = rearrange(indices, "(rn gn) bl gt -> rn bl gn gt", rn=rn)
        values = rearrange(values, "(rn gn) bl gt -> rn (bl gn gt)", rn=rn)
        offset = torch.arange(gn, device=device)[None, None, :, None] * ge
        indices = indices + offset
        indices = indices.flatten(1)

        indices_bin_o = F.one_hot(indices, en).type_as(x).mean(1).flatten()
        self.stat_o += indices_bin_o
        self.count += 1
        ddp = int(os.environ.get("RANK", -1)) != -1
        if ddp:
            dist.all_reduce(self.stat_o, op=dist.ReduceOp.AVG)
        self.stat = self.stat_o / self.count * en

        indices_bin = self.stat
        if m_stat is not None:
            of = config.stat_o_factor
            indices_bin = (self.stat - m_stat + 1) * (1 - of) + self.stat * of
        ...
        if not config.balance_loss_free:
            ...
            balance_loss = (indices_bin * values_bin).sum() * bf
...
class Model(nn.Module):
    def __init__(self, config: Config):
        ...
        if config.tied_vocab_emb:
            self.aux_model.vocab_emb.weight = self.lm_head.weight
    ...
    def forward(
        ...
        x, x_aux, doc_aux = self.aux_model.prepare(input, config)
        indices, values, balance_loss = self.aux_model(x_aux, doc_aux, config, m_stat)
    ...
    def params_count(self, config: Config):
        vocab_emb_params = config.main_dim * config.vocab_size
        aux_model_params = (
            sum(x.numel() for x in self.aux_model.parameters()) - vocab_emb_params
        )
        main_model_params = sum(x.numel() for x in self.main_model.parameters())
        total_params = sum(x.numel() for x in self.parameters())
        emb_params = total_params - aux_model_params - main_model_params
        params_result = {
        ...
        vocab_emb_a_params = vocab_emb_params
