...
aux_s_start = config.aux_short_start
aux_l_start = config.aux_long_start
aux_s_steps = config.aux_short_steps
aux_l_steps = config.aux_long_steps
...
last_time = time.time()
aux_model = None
aux_stat = torch.zeros(rn * en, device=device)
aux_count = torch.zeros(1, device=device)
...
for step in range(init_step + 1, num_steps + 1):
        ...
        with torch.no_grad():
            with amp_ctx:
                for _ in range(val_steps):
                    ...
                    step_val_loss = model(x, y, config, None)[1]
    ...
    with profile_ctx as prof:
            ...
            with sync_ctx:
                with amp_ctx:
                    ...
                    if aux_model is not None:
                        with torch.no_grad():
                            _, x_aux, doc = aux_model.prepare(x, config)
                            _ = aux_model(x_aux, doc, config, None)[0]
                            m_stat = aux_model.stat
                            del _
                    loss, step_main_loss, step_balance_loss = model(x, y, config, m_stat)
                    loss = loss / sum_steps
                    main_loss += step_main_loss / sum_steps
                    balance_loss += step_balance_loss / sum_steps
                    if config.scaler:
        ...
        print(f"stat: {raw_model.aux_model.stat}")
        if aux_model is not None:
            print(f"aux stat: {aux_model.stat}")
            print(f"diff: {raw_model.aux_model.stat - aux_model.stat}")
            aux_stat += aux_model.stat
            aux_count += 1
            aux_model.reset()
        raw_model.aux_model.reset()

    if config.aux_mode and step >= aux_s_start:
        s_cond = step < aux_l_start and (step - aux_s_start) % aux_s_steps == 0
        l_cond = step >= aux_l_start and (step - aux_l_start) % aux_l_steps == 0
        aux_change_cond = s_cond or l_cond

        if aux_change_cond:
            aux_model = copy.deepcopy(raw_model.aux_model)
            logger.log(logging.INFO, f"new aux model at step {step}")

        if aux_change_cond and aux_count > 0:
            avg_aux_stat = aux_stat / aux_count
            if ddp:
                dist.all_reduce(avg_aux_stat, op=dist.ReduceOp.AVG)
            logger.log(logging.INFO, f"aux_stat: {avg_aux_stat}, aux_count: {aux_count}")
            aux_stat -= aux_stat
            aux_count -= aux_count
