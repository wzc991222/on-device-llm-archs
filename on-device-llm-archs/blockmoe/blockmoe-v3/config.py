...
class Config:
    ...
    aux_group_wise_router: bool
    sole_group_wise_router: bool
    group_wise_coeff: bool
    router_group_num: int
    norm_routing: bool
    router_norm_value: float
    ...
    actual_group_expert_num: int = field(init=False)
    group_expert_num_per_token: int = field(init=False)
    group_router_size: int = field(init=False)
    group_router_mul_factor: int = field(init=False)
    ...
    def __post_init__(self):
        ...
        self.actual_group_expert_num = (
            self.group_expert_num
            if self.sole_group_wise_router
            else self.routed_expert_num
        )
        self.group_expert_num_per_token = (
            self.expert_num_per_token // self.router_group_num
        )
        self.group_router_size = (
            self.router_group_num if self.aux_group_wise_router else 1
        )
        self.group_router_mul_factor = (
            1 if self.aux_group_wise_router else self.router_group_num
        )
        self.total_layer_expert_num = self.routed_expert_num + self.expert_num_per_token
        self.router_size = self.moe_layer_num * self.group_router_size
