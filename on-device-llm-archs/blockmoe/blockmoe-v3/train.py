...
aux_accumulated_steps = config.aux_accumulated_steps
...
class DistributedDataLoader:
    ...
    def next_aux_batch(self):
        b, l, n = self.aux_batch_size, self.block_len, self.block_size
        bl = b * l
        tokens_len = len(self.tokens) - n - 1
        indices = random.sample(range(tokens_len), bl)
        offsets = np.arange(n)
        indices = (np.array(indices)[:, None] + offsets).ravel()
        buffer = self.tokens[indices]
        buffer = torch.tensor(buffer.astype(np.int32), dtype=torch.long)
        x = buffer.reshape(b, l * n)
        return x
...
for step in range(init_step + 1, num_steps + 1):
    ...
    with profile_ctx as prof:
        for micro_step in range(accumulated_steps):
            ...
            with sync_ctx:
                with amp_ctx:
                    loss, step_info = model(x, y, config)
        ...
        if not config.balance_loss_free:
            for micro_step in range(aux_accumulated_steps):
                aux_x = train_loader.next_aux_batch()
                aux_x = aux_x.to(device)
                sync_ctx = (
                    model.no_sync()
                    if ddp and micro_step < aux_accumulated_steps - 1
                    else nullcontext()
                )

                with sync_ctx:
                    with amp_ctx:
                        balance_loss = model(aux_x, aux_x, config, True)
                        balance_loss = balance_loss / aux_accumulated_steps
                        if config.scaler:
                            scaler.scale(balance_loss).backward()
                        else:
    ...
    if master_process:
        ...
        main_loss, norm_loss = info

        msg = (
            f"\nstep: {step}/{num_steps} "
            f"main_loss: {main_loss:.4f} "
            f"balance_loss: {balance_loss:.3f} "
            f"norm_loss: {norm_loss:.3f} "
