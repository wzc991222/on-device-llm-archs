...
main_b_loss_mode = config.main_balance_loss
...
for step in range(init_step + 1, num_steps + 1):
    ...
    with profile_ctx as prof:
        for micro_step in range(sum_steps):
            ...
            with sync_ctx:
                with amp_ctx:
                    loss, step_main_loss, step_main_b_loss = model(
                        x, y, config, False, main_b_loss_mode
                    )
                    loss = loss / sum_steps
        ...
        for micro_step in range(aux_sum_steps):
            ...
            with sync_ctx:
                with amp_ctx:
                    loss = model(aux_x, None, config, True, True)
    ...
    if master_process:
        ...
        msg = (
            ...
            f"main_balance_loss: {main_b_loss:.4f} "
            f"aux_balance_loss: {aux_b_loss:.4f} "
        ...
        wandb_stat = {
            "main_loss": main_loss,
            "main_balance_loss": main_b_loss,
            "aux_balance_loss": aux_b_loss,
            "step_time": time_ms,
