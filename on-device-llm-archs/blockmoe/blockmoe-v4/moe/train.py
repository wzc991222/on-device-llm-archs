...
for step in range(init_step + 1, num_steps + 1):
        ...
        with torch.no_grad():
            with amp_ctx:
                for _ in range(val_steps):
                    ...
                    step_val_loss = model(x, y, config)[1]
    ...
    with profile_ctx as prof:
        for micro_step in range(sum_steps):
            ...
            with sync_ctx:
                with amp_ctx:
                    loss, step_main_loss, step_balance_loss = model(x, y, config)
                    loss = loss / sum_steps
        ...
        if ln > 0:
            layer = raw_model.moe_layers[1]
            avg_stat = layer.stat / layer.count
            logger.log(logging.INFO, f"indices: {avg_stat}")
            logger.log(logging.INFO, f"indices max: {avg_stat.max()}")
            logger.log(logging.INFO, f"logits_max: {layer.logits_max / layer.count}")
            logger.log(logging.INFO, f"logits_mean: {layer.logits_mean / layer.count}")
