...
def grouped_gemm_forward(
    ...
    if type == "attn":
        repeat_num = config.attn_repeat_num
    if type == "attn_o":
        repeat_num = config.attn_o_repeat_num
    if type == "ffn":
        repeat_num = config.ffn_repeat_num

    if type == "attn_o":
        x_repeat = rearrange(x, "b s (n e) -> (b s n) e", n=repeat_num)
    else:
        x_repeat = repeat(x, "b s d -> (b s n) d", n=repeat_num)

    index_s = torch.argsort(index, stable=False)
    index_freq = torch.bincount(index, minlength=block.shape[-3])
    if gmm_option:
    ...
    x_st = y_s1 = y_s1s = y_s2 = y_s3 = None
    ...
    if type == "ffn":
        ...
        if for_loop_option:
            x_s = torch.split(x_s, index_freq, 0)
            block_split = torch.unbind(block, 0)
            y_s = [x @ b for x, b in zip(x_s, block_split)]
            y_s = torch.cat(y_s, 0)
...
def grouped_gemm_func_original(
    ...
    y_s, repeat_num, index_s = grouped_gemm_forward(x, block, index, config, type)
...
class GroupedGEMM(Function):
    ...
    def forward(
        ...
        y_output = grouped_gemm_func_original(x, block, score, index, config, type)
        ctx.save_for_backward(x, block, score, index)
        ctx.info = (config, type)

        return y_output

    @staticmethod
    def backward(ctx, grad_output: torch.Tensor):
        x, block, score, index = ctx.saved_tensors
        config, type = ctx.info
        grad_block_option = ctx.needs_input_grad[1]
        x_list, y_list, index_list, info_list = grouped_gemm_forward(
            x, block, index, config, type, False
        )
        x_s, x_st = x_list
        ...
        if type == "attn":
            grad_o = rearrange(grad_output, "b s (n e) -> (b s n) e", n=repeat_num)
        else:
        ...
        if type == "ffn":
            ...
            if for_loop_option:
                if grad_block_option:
                    ...
                    grad_block_3 = [
                        y1 @ y2 for y1, y2 in zip(grad_y_st, y_s3) if y_s3.shape[0] != 0
                    ]
            ...
            if for_loop_option:
                ...
                if grad_block_option:
                    grad_block_1 = [
                        x @ y for x, y in zip(x_st, grad_y_s1) if x_st.shape[1] != 0
                    ]
                    grad_block_2 = [
                        x @ y for x, y in zip(x_st, grad_y_s2) if x_st.shape[1] != 0
                    ]
            ...
            if grad_block_option:
                if gmm_option:
                    grad_block = torch.stack(
                        (grad_block_1, grad_block_2, grad_block_3), 0
                    )
                if for_loop_option:
                    grad_block = torch.zeros_like(block)
                    index_freq_mask = index_freq > 0
                    grad_block[:, index_freq_mask] = torch.stack(
                        (grad_block_1, grad_block_2, grad_block_3), 0
                    )
            ...
            if for_loop_option:
                ...
                if grad_block_option:
                    grad_block_list = [
                        x @ y for x, y in zip(x_st, grad_y_s) if x_st.shape[1] != 0
                    ]
                    grad_block = torch.zeros_like(block)
                    grad_block[index_freq_mask] = torch.stack(grad_block_list, 0)
                del x_st
                block_t = torch.unbind(block.transpose(1, 2), 0)
                grad_x_s = [y @ b for y, b in zip(grad_y_s, block_t)]
        ...
        if type == "attn_o":
            grad_x = rearrange(
                grad_x, "(b s n) e -> b s (n e)", s=config.seq_len, n=repeat_num
            ).type_as(x)
        else:
            grad_x = rearrange(
        ...
        return grad_x, grad_block, grad_score, None, None, None
