...
predict_sum_steps = config.predict_sum_steps
main_sum_steps = sum_steps - predict_sum_steps
...
mb_loss = config.main_balance_loss
...
for step in range(init_step + 1, num_steps + 1):
        ...
        with torch.no_grad():
            with amp_ctx:
                for _ in range(val_steps):
                    ...
                    step_val_loss = model(x, y, config, False, False, False)[1]
            ...
            with torch.no_grad():
                with amp_ctx:
                    for _ in range(val_steps):
                        x, y = val_loader.next_batch()
                        x, y = x.to(device), y.to(device)
                        step_val_loss = model(x, y, config, False, True, False)[1]
                        val_loss += step_val_loss / val_steps
            if ddp:
                dist.all_reduce(val_loss, op=dist.ReduceOp.AVG)
            if master_process:
                logger.log(
                    logging.INFO, f"step:{step}/{num_steps} p_val_loss:{val_loss:.4f}"
                )
                wandb_logger.log({"p_val_loss": val_loss}, step=step)
    ...
    with profile_ctx as prof:
        for micro_step in range(main_sum_steps):
            x, y = train_loader.next_batch()
            x, y = x.to(device), y.to(device)
            sync_ctx = (
                model.no_sync()
                if ddp and micro_step < main_sum_steps - 1
                else nullcontext()
            )
            with sync_ctx:
                with amp_ctx:
                    (
                        ...
                        step_main_pb_loss,
                        step_rp_loss,
                    ) = model(x, y, config, False, False, mb_loss)
                    loss = loss / sum_steps
                    main_loss += step_main_loss / main_sum_steps
                    main_b_loss += step_main_b_loss / sum_steps
                    main_pb_loss += step_main_pb_loss / sum_steps
                    router_p_loss += step_rp_loss / sum_steps
                    if config.scaler:
                        scaler.scale(loss).backward()
                    else:
                        loss.backward()

        for micro_step in range(predict_sum_steps):
            x, y = train_loader.next_batch()
            x, y = x.to(device), y.to(device)
            sync_ctx = (
                model.no_sync()
                if ddp and micro_step < predict_sum_steps - 1
                else nullcontext()
            )
            with sync_ctx:
                with amp_ctx:
                    (
                        ...
                        step_main_pb_loss,
                        step_rp_loss,
                    ) = model(x, y, config, False, True, mb_loss)
                    loss = loss / sum_steps
                    main_p_loss += step_main_p_loss / predict_sum_steps
                    main_b_loss += step_main_b_loss / sum_steps
                    main_pb_loss += step_main_pb_loss / sum_steps
                    router_p_loss += step_rp_loss / sum_steps
                    if config.scaler:
        ...
        if config.main_balance_loss:
            ...
            if config.aux_predict_model:
                aux_p_model = raw_model.aux_predict_model
                avg_p_stat = aux_p_model.stat / aux_p_model.count
                avg_p_logits_max = aux_p_model.logits_max / aux_p_model.count
                avg_p_logits_mean = aux_p_model.logits_mean / aux_p_model.count
        ...
        for micro_step in range(aux_sum_steps):
            ...
            with sync_ctx:
                with amp_ctx:
                    loss = model(aux_x, None, config, True, False, True)
                    aux_b_loss += loss.detach() / aux_sum_steps
        ...
        if ddp:
            ...
            dist.all_reduce(main_p_loss, op=dist.ReduceOp.AVG)
            dist.all_reduce(main_b_loss, op=dist.ReduceOp.AVG)
            dist.all_reduce(main_pb_loss, op=dist.ReduceOp.AVG)
            dist.all_reduce(router_p_loss, op=dist.ReduceOp.AVG)
            dist.all_reduce(aux_b_loss, op=dist.ReduceOp.AVG)
    ...
    if master_process:
        ...
        msg = (
            ...
            f"main_p_loss: {main_p_loss:.4f} "
            f"main_b_loss: {main_b_loss:.4f} "
            f"main_pb_loss: {main_pb_loss:.4f} "
            f"router_p_loss: {router_p_loss:.4f} "
            f"aux_b_loss: {aux_b_loss:.4f} "
        ...
        wandb_stat = {
            "main_loss": main_loss,
            "main_p_loss": main_p_loss,
            "main_b_loss": main_b_loss,
            "main_pb_loss": main_pb_loss,
            "router_p_loss": router_p_loss,
            "aux_b_loss": aux_b_loss,
            "step_time": time_ms,
