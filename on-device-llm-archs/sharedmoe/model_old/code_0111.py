...
from grouped_gemm.ops import gmm # type: ignore
...
class Config:
    ...
    training_update_freq: int = 5
    update_cutoff_point: float = 0.9
    balance_factor: float = 1e-3
    router_z_factor: float = 1
    vec_balance_factor: float = 1e-1
    vec_router_z_factor: float = 1
...
def router(
    ...
    Dict[str, torch.Tensor], # router_output: dict, indices [t * m_i * m_o * i], probs [t * m_i * m_o * i, 1] or indices [t * f_n * k * i], probs [t * f_n * k * i, 1], i = index_num
    ...
    c_router_indices = torch.argmax(torch.matmul(x_queries, cluster_router), -1)  # [t * m, v] @ [v, n] = [t * m, n] -> argmax -> [t * m]

    order_indices = torch.argsort(c_router_indices, stable = False) # [t * m]
    x_queries_sorted = x_queries[order_indices, :] # x_queries: [t * m, v]
    index_freqs, filter = permute_info(c_router_indices) # [n_v] * 2, n_v = vec_num_sqrt valid < n
    router_blocks = torch.transpose(vector_pool.vec[module]['vec'][layer_id, filter, :, :], 1, 2).contiguous().to(torch.bfloat16) # [n_v, v, n_e]
    router_permuted = gmm(x_queries_sorted, router_blocks, index_freqs.to('cpu'), trans_b = False) # [t * m, v] gmm ([n_v, v, n_e], [n_v]) -> [t * m, n_e]
    
    router = torch.zeros_like(router_permuted) # [t * m, n_e]
    router[order_indices, :] = router_permuted # [t * m, n_e]

    router_values, router_indices = torch.topk(router, top_k, dim = -1) # router_values: [t * m, k], router_indices: [t * m, k], k = top_k
    
    c_router_indices = repeat(c_router_indices, 'tm -> tm k', k = top_k) # [t * m, k]

    block_indices = vector_pool.vec_info[module]['block_indices'][layer_id, c_router_indices, router_indices, :] # [l, n, n_e, i] -> [t * m, k, i], l = layer_num, i = index_num
    ...
    sigmoid_factors = torch.max(sigmoid_factors, torch.zeros(5, device = sigmoid_factors.device, dtype = torch.bfloat16))

    router_probs = 1 / (torch.exp(-1 * sigmoid_factors[0] * router_values) + sigmoid_factors[1]) # [t * m, k]
    router_probs_r = repeat(router_probs, 'tm k -> tm k c', c = vector_pool.index_num) # [t * m, k] -> [t * m, k, i]
    gain_probs = 1 / (torch.exp(-1 * sigmoid_factors[2] * gain_factors) + sigmoid_factors[3]) # [t * m, k, i]
    block_probs = router_probs_r * gain_probs * sigmoid_factors[4] # [t * m, k, i]
    block_probs = block_probs.to(torch.bfloat16)

    block_indices = rearrange(block_indices, 'tm k c -> (tm k c)') # [t * m, k, i] -> [t * m * k * i]
    ...
    if vector_pool.router_detach:
        ...
        router_permuted_detach = gmm(x_queries_sorted.detach(), router_blocks, index_freqs.to('cpu'), trans_b = False) # [t * m, v] gmm ([n_v, v, n_e], [n_v]) = [t * m, n_e]
        router[order_indices, :] = router_permuted_detach # [t * m, n_e]
    ...
    balance_loss_1 = torch.sum(router_indices_bincount * router_values_bincount)
    balance_loss_2 = router_indices.numel()
    balance_loss_3 = torch.sum(router_values_bincount)
    balance_loss_4 = router_indices_bincount.numel()
    balance_loss = balance_loss_1 / (balance_loss_2 * balance_loss_3) * balance_loss_4
    print(module, layer_id)
    print(balance_loss.item(), balance_loss_1.item(), balance_loss_2, balance_loss_3.item(), balance_loss_4)

    router_z_loss_1 = torch.sum(router_values ** 2)
    router_z_loss_2 = router_values.numel()
    router_z_loss = router_z_loss_1 / router_z_loss_2
    print(router_z_loss.item(), router_z_loss_1.item(), router_z_loss_2)
...
class ParamVectorPool(nn.Module):
    ...
    def __init__(self, config: Config) -> None:
        ...
        for module, module_parent in zip(config.module_list, config.module_parent):
            ...
            g = torch.Generator()
            g.manual_seed(42)
            module_vec_info['block_indices'] = torch.randint(1, block_num, (layer_num, vec_num_s, vec_num_se, index_num), dtype = torch.int32, generator = g).to(self.device) # [l, n, n_e, i]
            module_vec_info['candidates'] = [None for _ in range(layer_num)]
            module_vec_info['repeat_init_vec'] = torch.cat((torch.zeros(vec_num_s, vec_dim + 2 * index_num), torch.arange(vec_num_s)[:, None]), -1).to(self.device) # [n, v_cat], v_cat = v + 2i +1
            module_vec_info['vec_num'] = (vec_num_s, vec_num_se)
            module_vec_info['vec_dim'] = vec_dim
            module_vec_info['vec_cat_dim'] = vec_dim + 2 * index_num + 1
        ...
        self.index_dtye = self.vec_info['ffn']['block_indices'].dtype
    ...
    def get_info(self) -> List[List]:
        ...
        for module in self.module_list:
            for i in range(self.layer_num):
                ...
                vec_index = self.vec_info[module]['block_indices'][i, :, :, 0] # [n, n_e]
    ...
    def get_vec_router_loss(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        vec_balance_loss = torch.tensor(0.0, device = self.device)
        vec_router_z_loss = torch.tensor(0.0, device = self.device)

        for module in config.module_list:

            vec_clusters = self.vec[module]['vec_clusters'] # [l, n, v]
            vec = self.vec[module]['vec'] # [l, n, n_e, v]
            block_indices = self.vec_info[module]['block_indices'][:, :, :, 0] # [l, n, n_e]
    ...
    def vec_cat_func(self, init: bool) -> Dict[str, torch.Tensor]:
        ...
        for module in self.module_list:
            vec_cat = torch.cat((
                ...
                self.vec_info[module]['block_indices'].to(self.vec_dtype), 
    ...
    def cand_num_list_init(self, world_size: int) -> None:
        ...
        self.cand_num_list = [torch.zeros(list_num, 2, device = self.device, dtype = torch.int) for _ in range(world_size)] # (world_size, ) list: [(list_num, 2), ...]
    ...
    def vec_filling(self, ddp: bool) -> None:
        ...
        for i, module in enumerate(self.module_list):
            ...
            self.vec_info[module]['block_indices'] = module_vec_list[i][:, :, :, vec_dim + self.index_num : vec_dim + 2 * self.index_num].to(self.index_dtye) # [l, n, n_e, i]
    ...
    def ml_update( # ml: module & layer
        ...
        if not init:
            ...
            vec = torch.cat((vec, vec_old_cand), 0) # [n * n, v_cat]
        ...
        vec_pad = torch.repeat_interleave(self.vec_info[module]['repeat_init_vec'], bincounts, dim = 0) # [(n_max - n) * n, v_cat]
        vec_updated = torch.cat((vec, vec_pad), 0) # [n * n_max, v_cat]
        vec_updated_index = vec_updated[:, -1] # [n * n_max]
        vec_updated = vec_updated[torch.argsort(vec_updated_index, stable = True)] # [n * n_max, v_cat]
        
        vec_updated = rearrange(vec_updated, '(n1 n2) v -> n1 n2 v', n1 = vec_num_s) # [n * n_max, v_cat] -> [n, n_max, v_cat]
        vec_cand = vec_updated[:, vec_num_se:, :]
        vec_cand = rearrange(vec_cand, 'n1 n2 v -> (n1 n2) v') # [n * (n_max - n_e), v_cat]
    ...
    def update(
        ...
        for module in self.module_list:
            ...
            module_vec_updated = torch.zeros(self.layer_num, *self.vec_info[module]['vec_num'], self.vec_info[module]['vec_cat_dim'], device = self.device) # [l, n, n_e, v_cat]
            module_vec_cand = []
            ...
            for i in range(self.layer_num):
                ...
                module_vec_updated[i] = ml_vec_updated
    ...
    def update_ddp(
        ...
        vec_cand_num = torch.zeros(list_num, 2, device = self.device, dtype = torch.int) # [list_num, 2]
    ...
    def vec_init(
        ...
        for module in self.module_list:
            ...
            router_indices[module] = torch.zeros(*vec_dict[module].shape[:-1], device = self.device, dtype = torch.int) # [l, n * n]
...
class Linear(nn.Module):
    ...
    def forward(
        ...
        vector_pool: ParamVectorPool,
        router: Callable,
        router_result: Optional[Dict[str, Dict[str, torch.Tensor]]], # (sublayer_num, 2) dict or None, indices [t * s_io * i], probs [t * s_io * i, 1], s_io = (m_io)_sum
        ...
        Optional[Dict[str, torch.Tensor]], # router_output: indices [t * s_io * i], probs [t * s_io * i, 1] or None
        torch.Tensor, # balance_loss
        torch.Tensor, # router_z_loss
    ]:
        ...
        x_cat = torch.zeros(self.seq_len, 0, device = device, dtype = dtype)
        ...
        if self.have_router:
            ...
            with router_ctx:
                ...
                if self.info_fusion_router:
                    ...
                    external = {'blockwise': torch.tensor(0, device = device, dtype = dtype), 'multihead': torch.tensor(0, device = device, dtype = dtype)}
                ...
                print(f'q_blockwise original mean: {q_blockwise.mean()}')
                q_blockwise = q_blockwise.to(dtype)
                print(f'q_blockwise new mean: {q_blockwise.mean()}')
                
        
                q_multihead = torch.zeros(self.seq_len, 0, device = device, dtype = dtype)
                ...
                print(f'q_multihead original mean: {q_multihead.mean()}')
                q_multihead = q_multihead.to(dtype)
                print(f'q_multihead new mean: {q_multihead.mean()}')
                ...
                for i, module in enumerate(self.sublayer_modules):
                    ...
                    q_blockwise_module = repeat(q_blockwise[:, index_il : index_ir], 't (m_i v) -> (t m_i m_o) v', v = self.vec_dim, m_o = m_o_size) # [t, m_i * v] -> [t * m_i * m_o, v]
                    q_multihead_module = repeat(q_multihead[:, index_ol : index_or], 't (m_o v) -> (t m_i m_o) v', v = self.vec_dim, m_i = m_i_size) # [t, m_o * v] -> [t * m_i * m_o, v]
                    x_query_cat = torch.cat((q_blockwise_module, q_multihead_module), -1)
                    print(f'linear module {module} x_query mean: {x_query_cat.mean()}')
                    x_queries[module] = x_query_cat # [t * m_i * m_o, 2 * v]
                ...
                router_output_index = torch.zeros(0, device = device, dtype = torch.int)
                router_output_prob = torch.zeros(0, 1, device = device, dtype = dtype)
                top_k = 1

                for module, x_query in x_queries.items(): # x_queries: dict, each [t * m_i * m_o, 2 * v]
                    m_router_output, m_balance_loss, m_router_z_loss = router(x_query, vector_pool, module, self.layer_id, top_k) # [t * m_i * m_o * i], [t * m_i * m_o * i, 1]
                    router_output_index = torch.cat((router_output_index, m_router_output['indices']), 0) # [(t * m_i * m_o)_sum * i] = [t * s_io * i]
        ...
        if not self.vanilla_matmul: 
            ...
            with main_ctx:
                ...
                block_indices = router_result[self.sublayer]['indices'] # [t * s_io * i]
                ...
                x_input = torch.zeros(0, hidden_block_dim, device = device, dtype = dtype)
                ...
                order_indices = torch.argsort(block_indices, stable = False) # [t * s_io * i]
                x_sorted = x_input[order_indices, :] # [t * s_io * i, d_h]
                ...
                blocks = block_pool.weight_pool[sublayer][filter, :, :].contiguous().to(dtype) # [vis, d_h, d]
                x_permuted = gmm(x_sorted, blocks, index_freq.to('cpu'), trans_b = False) # [t * s_io * i, d_h] gmm ([vis, d_h, d], [vis]) = [t * s_io * i, d]

                x_unpermuted = torch.zeros_like(x_permuted) # [t * s_io * i, d]
                x_unpermuted[order_indices, :] = x_permuted # [t * s_io * i, d]
                if self.bias:
                    bias = torch.index_select(block_pool.bias_pool[sublayer], 0, block_indices) # [t * s_io * i, d]
                    x_unpermuted += bias # [t * s_io * i, d]

                x_unpermuted = x_unpermuted * block_probs # [t * s_io * i, d]
                ...
                for i, module in enumerate(self.sublayer_modules):
                    ...
                    x_output_module = reduce(x_unpermuted[index_l : index_r, :], '(t m_i m_o i) d -> t (m_o d)', 'sum', m_i = m_i_size, m_o = m_o_size, i = index_num)
                    x_output[module] = x_output_module.to(dtype) # [t * m_i * m_o * i, d] -> [t, m_o * d]  
    ...
    def forward(
        ...
        if self.have_router:
            ...
            with router_ctx:
                ...
                if self.info_fusion_router:
                    ...
                    external = torch.tensor(0, device = device, dtype = dtype)
                ...
                print(f'x_queries original mean: {x_queries.mean()}')
                x_queries = x_queries.to(dtype)
                router_output, balance_loss, router_z_loss = router(x_queries, vector_pool, 'ffn', self.layer_id, self.top_k) # [t * f_n * k * i], [t * f_n * k * i, 1]
        ...
        if not self.vanilla_matmul:
            ...
            with main_ctx:
                ...
                assert router_result is not None
                block_indices = router_result['ffn']['indices'] # [t * f_n * k * i]
                ...
                order_indices = torch.argsort(block_indices, stable = False) # [t * f_n * k * i]
                x_sorted = x_input[order_indices, :]# [t * f_n * k * i, 2d]

                index_freq, filter = permute_info(block_indices) # [vis] * 2, vis = valid index size

                blocks = block_pool.weight_pool['ffn'][filter, :, :, :].to(dtype) # [vis, 3, d, d_f]
                block_3t = torch.transpose(blocks[:, 2, :, :], 1, 2).contiguous() # [vis, d_f, d]
                x_permuted_1 = gmm(x_sorted[:, :self.block_dim].contiguous(), blocks[:, 0, :, :].contiguous(), index_freq.to('cpu'), trans_b = False) # [t * f_n * k * i, d] gmm ([vis, d, d_f] , [vis]) = [t * f_n * k * i, d_f]
                x_permuted_2 = gmm(x_sorted[:, self.block_dim:].contiguous(), blocks[:, 1, :, :].contiguous(), index_freq.to('cpu'), trans_b = False) # [t * f_n * k * i, d_f]
                x_permuted = F.silu(x_permuted_1) * x_permuted_2 # [t * f_n * k * i, d_f]
                x_permuted = gmm(x_permuted, block_3t, index_freq.to('cpu'), trans_b = False) # [t * f_n * k * i, d]

                x_unpermuted = torch.zeros_like(x_permuted) # [t * f_n * k * i, d]
                x_unpermuted[order_indices, :] = x_permuted # [t * f_n * k * i, d]
                if self.bias:
                    bias = torch.index_select(block_pool.bias_pool['ffn'], 0, block_indices) # [t * f_n * k * i, d]
                    x_unpermuted += bias # [t * f_n * k * i, d]
                x_unpermuted = x_unpermuted * block_probs

                x_output = reduce(x_unpermuted, '(t f_n k i) d -> t (f_n d)', 'sum', f_n = self.ffn_size, k = self.top_k, i = index_num) # [t, f_n * d]
                x_output = x_output.to(dtype)
...
class RotaryEmb(nn.Module):
    ...
    def forward(self, xq: torch.Tensor, xk: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.seq_len != seq_len:
            ...
            self.freqs_cis = torch.polar(torch.ones_like(outer_freqs), outer_freqs)[None, None, :, :].to(xq.device) # [1, 1, t, a_n * d/2]
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if self.qk_norm:
            xq = F.normalize(xq, dim = -1, eps = self.norm_eps).to(dtype) # [1, h_q, t, a_n * d]
            xk = F.normalize(xk, dim = -1, eps = self.norm_eps).to(dtype) # [1, h_k, t, a_n * d]
        
        xq, xk = rotary_emb(xq, xk) # [1, h_q, t, a_n * d], [1, h_k, t, a_n * d]
        y_output = flex_attention(xq, xk, xv, block_mask = block_mask, enable_gqa = self.gqa, kernel_options = self.kernel_options) # [1, h_q, t, a_n * d]
...
class Layer(nn.Module):
    ...
    def __init__(self, config: Config, layer_id: int) -> None:
        ...
        device = f'cuda:{torch.cuda.current_device()}'
        dtype = torch.bfloat16
        self.norm_eps = torch.tensor(config.norm_eps, device = device, dtype = dtype)
    ...
    def forward(
        ...
        norm = torch.linalg.vector_norm(x, dim = -1, keepdim = True, dtype = dtype) + self.norm_eps # [t, 1]
        ...
        residual_coeff = self.residual_coeff if self.residual_factor else torch.tensor(1.0, device = device, dtype = dtype)
        if not self.residual_norm:
            residual_norm_coeff = torch.tensor(1.0, device = device, dtype = dtype)
        x_4 = output[4][0]

        x_output = x * residual_norm_coeff * residual_coeff + x_4['output']
        x_output = x_output.to(dtype)

        router_output = {sublayer: output[i][-3] for i, sublayer in enumerate(self.sublayers)} if output[0][-3] else None
...
class Model(nn.Module):
    ...
    def init_weights(self) -> None:
        ...
        for name, param in self.vector_pool.named_parameters():
            if name[-3:] == 'vec' or 'vec_clusters' in name:
                nn.init.trunc_normal_(param, mean = 0, std = std, a = -3 * std, b = 3 * std, generator = self.g)
            if 'gain_factors' in name or 'sigmoid_factors' in name:
                nn.init.ones_(param)
    ...
    def forward(self, idx: torch.LongTensor, targets: torch.Tensor) -> Tuple[Optional[torch.Tensor], torch.Tensor]:
        ...
        seq_len = len(idx)
        block_mask = create_block_mask(doc_causal_mask, None, None, seq_len, seq_len, device = device, _compile = True)
        ...
        accumulated_num = len(self.module_list) * self.layer_num
        balance_loss = balance_loss / accumulated_num
        router_z_loss = router_z_loss / accumulated_num
...
def configure_optimizers(model: Model, config: Config):
    ...
    if config.zero_optimizer:
        optimizer = ZeroRedundancyOptimizer(optim_groups, optimizer_class = torch.optim.AdamW, betas = betas, foreach = True)
    else:
...
parser.add_argument("--seq_len", type = int, default = 4 * 1024)
...
for step in range(num_steps + 1):
    ...
    if (last_step or (config.val_every > 0 and step > 0 and step % config.val_every == 0)):
    ...
    profile_ctx = profile(activities = [ProfilerActivity.CPU, ProfilerActivity.CUDA], record_shapes = True, profile_memory = True, with_stack = True) if step == 3 else nullcontext()
    ...
    with profile_ctx as prof:
        ...
        for micro_step in range(config.accumulated_steps):
            ...
            sync_ctx = model.no_sync() if ddp and micro_step < config.accumulated_steps - 1 else nullcontext()

            with sync_ctx:
                with amp_ctx:
                    t_0 = time.time()
                    _, loss, a_loss = model(x, y)
                    t_1 = 1000 * (time.time() - t_0)
    ...
    if master_process:
        ...
        sigmoid_factors = torch.zeros(5, device = device)
        for module in config.module_list:
        ...
        print0(f'step:{step}/{num_steps} train_loss:{train_loss.item():.4f} time:{total_time_ms / 1000:.0f}s step_time:{running_avg_time:.1f}ms forward_time:{t_1:.1f}ms {running_avg_tokens:.0f} t/s  norm:{grad_norm:.3f} a_loss:({a_loss_list[0]:.3f}, {a_loss_list[1]:.3f}, {a_loss_list[2]:.3f}, {a_loss_list[3]:.3f}) vec_info: ({cand_p_avg:.3f}, {max_c_vec_p_avg:.3f}, {min_c_vec_p_avg:3f}) coeff: ({residual_coeff_avg:.3f}, {residual_norm_coeff_avg:.3f})')

        train_log.append([step, train_loss.item(), total_time_ms / 1000, running_avg_time, t_1, running_avg_tokens, grad_norm, cand_p_avg, max_c_vec_p_avg, min_c_vec_p_avg, residual_coeff_avg, residual_norm_coeff_avg] + a_loss_list + sigmoid_factors)
        ...
        if step % config.log_freq == 0:
            ...
            df = pd.DataFrame(train_log, columns = ['step', 'train_loss', 'total_time(s)', 'step_time(ms)', 'forward_time', 'token_speed(t/s)', 'grad_norm', 'cand_p_avg', 'max_c_vec_p_avg', 'min_c_vec_p_avg', 'residual_coeff_avg', 'residual_norm_coeff_avg', 'balance_loss', 'router_z_loss', 'vec_balance_loss', 'vec_router_z_loss', 'sigmoid_factors_1', 'sigmoid_factors_2', 'sigmoid_factors_3', 'sigmoid_factors_4', 'sigmoid_factors_5'])
