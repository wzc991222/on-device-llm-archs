...
class Query(nn.Module):
    def __init__(
        ...
        self.n_lr = main_router_block_num * linear_type1_multihead_num
        # n_lr
        self.n_fr = layer_ffn_router_block_num
    ...
    def forward(
        ...
        device = x_linear_type1.device
        dtype = x_linear_type1.dtype
        ...
        linear_type1_len = self.config.seq_len * self.n_lr * self.n_1m
        linear_type2_len = self.config.seq_len * self.n_lr * self.n_2m
        linear_len = linear_type1_len + linear_type2_len
        ffn_len = self.config.seq_len * self.n_fr
        query_len = linear_len + ffn_len

        query = torch.empty(query_len, self.v, device=device, dtype=dtype)
        # [n_lm * s * n_lr + s * n_fr, v]

        if external is not None:
            query[:linear_type1_len] = (
                (
                    x_linear_type1_blockwise
                    + x_linear_type1_fusion
                    + self.linear_type1_b.to(self.dtype)
                    + external["linear_type1"]
                )
                .transpose(0, 1)
                .flatten(0, -2)
            )

            query[linear_type1_len : linear_type1_len + linear_type2_len] = (
                (
                    x_linear_type2_blockwise
                    + x_linear_type2_fusion
                    + self.linear_type2_b.to(self.dtype)
                    + external["linear_type2"]
                )
                .transpose(0, 1)
                .flatten(0, -2)
            )

            query[linear_type1_len + linear_type2_len :] = (
                x_ffn_blockwise
                + x_ffn_fusion
                + self.ffn_b.to(self.dtype)
                + external["ffn"]
            ).flatten(0, -2)
            ...
            query[linear_type1_len + linear_type2_len :] = (
                x_ffn_blockwise + x_ffn_fusion + self.ffn_b.to(self.dtype)
            ).flatten(0, -2)

        return query, linear_len  # query: [n_lm * s * n_lr + s * n_fr, v]
...
class Router(nn.Module):
    ...
    def forward(
        self,
        query: torch.Tensor,  # [n_lm * s * n_lr + s * n_fr, v]
        linear_len: int,
    ) -> Tuple[Tuple, Tuple]:
        linear_cluster_values, linear_cluster_indices_original = torch.max(
            torch.bmm(
                query[:linear_len].reshape(self.n_lm, -1, self.config.vec_dim),
                self.vector_pool.linear_vec["vec_clusters"][self.layer_id],
            ),
            -1,
        )
        ...
        ffn_cluster_values, ffn_cluster_indices = torch.max(
            torch.matmul(
                query[linear_len:],
                self.vector_pool.ffn_vec["vec_clusters"][self.layer_id],
            ),
            -1,
        )
        ...
        linear_vec_scores = rearrange(
            ...
            "(n_lm s n_lr) n_cv -> s n_lm n_lr n_cv",
            s=self.config.seq_len,
            n_lm=self.n_lm,
        )
        # [s, n_lm, n_lr, n_cv]
...
class Layer(nn.Module):
    ...
    def forward(
        ...
        if not self.vanilla_matmul:
            ...
            n_mi = int(self.config.main_dim / d_li)  # n_mi = linear_main_block_in_num
            n_ci = int(
                self.config.compressed_dim / d_li
            )  # n_ci = linear_c_block_in_num

            x_linear_type1 = torch.empty(
                self.config.seq_len,
                self.n_1m,
                n_mi,
                d_li,
            ...
            x_linear_type1[:, :2, ...] = repeat(
                x_norm,
                "s (n_mi d_li) -> s 2 n_mi d_li",
                d_li=d_li,
            )
            # [s, 2, n_mi, d_li]
            x_1_in = x_linear_type1[:, :2, ...]
            ...
            x_linear_type2[:, :3, ...] = repeat(
                x_1_out[:, 0, :],
                "s (n_ci d_li) -> s 3 n_ci d_li",
                d_li=d_li,
            )
            x_linear_type2[:, 3:5, ...] = repeat(
                x_1_out[:, 1, :],
                "s (n_ci d_li) -> s 2 n_ci d_li",
                d_li=d_li,
            )
            x_2_in = x_linear_type2[:, :5, ...]
            # [s, 5, n_ci, d_li]
            ...
            # [s, 1, n_fr, 2 * d_f]
            x_linear_type1[:, 2, ...] = rearrange(
                self.attention(x_attn_in, block_mask).squeeze(1),
                "s (n_mi d_li) -> s n_mi d_li",
                d_li=d_li,
            )
            # [s, d_m] -> [s, n_mi, d_li]
            ...
            x_linear_type1[:, 3, ...] = rearrange(
                x_ffn_out.squeeze(1),
                "s (n_mi d_li) -> s n_mi d_li",
                d_li=d_li,
            )

            x_3_in = x_linear_type1[:, 2:, ...]
            ...
            x_linear_type2[:, 5, ...] = rearrange(
                torch.sum(x_3_out, 1, dtype=self.dtype),
                "s (n_ci d_li) -> s n_ci d_li",
                d_li=d_li,
            )
            x_4_in = x_linear_type2[:, 5, ...].unsqueeze(1)
            # [s, 1, n_ci, d_li]
        ...
        if not self.vanilla_matmul:
            x_linear_type1 = x_linear_type1.flatten(2)
            # [s, n_1m, d_m]
            x_linear_type2 = x_linear_type2.flatten(2)
            # [s, n_2m, d_c]
        else:
        ...
        if self.have_router:
            query, linear_len = self.query(
                x_linear_type1, x_linear_type2, x_ffn, external
            )
            # [n_lm * s * n_lr + s * n_fr, v]
            next_router_results, aux_loss = self.router(query, linear_len)
