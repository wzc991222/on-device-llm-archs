        ...
        if not init:
            ...
            if vec_indices_bin_zero_num > 0:
                ...
                c_mean_prob = torch.mean(
                    vec_indices_bin_non_zero / vec_probs_bin_non_zero
                ).item()
            else:
...
parser.add_argument("--cooldown_steps", type=int, default=50)
...
for step in range(num_steps + 1):
    ...
    with profile_ctx as prof:
        ...
        for micro_step in range(config.accumulated_steps):
            ...
            with sync_ctx:
                with amp_ctx:
                    step_train_loss, loss, step_c_aux_loss, step_vec_aux_loss = model(
                        x, y
                    )
