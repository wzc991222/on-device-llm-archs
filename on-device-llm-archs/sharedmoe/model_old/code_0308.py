...
class Config:
    ...
    adam_eps: float = 1e-8
    window_len: int = 4096
    c_token_balance_factor: float = 0.01
    vec_token_balance_factor: float = 0.01
    c_vec_balance_factor: float = 0.2
    c_ffn_balance_ratio: float = 4.0
    vec_ffn_balance_ratio: float = 1.5
    update_ffn_balance_ratio: float = 1.5
    c_ffn_balance_ratio_sep: float = 0.3
    vec_ffn_balance_ratio_sep: float = 0.1
    update_ffn_balance_ratio_sep: float = 0.3
    ...
    global_linear_block_num: int = 1150
    ffn_block_num: int = 600
    linear_cluster_num: int = 64
    ffn_cluster_num: int = 128
    c_vec_num: int = 64
    valid_c_vec_num: int = 64

    fixed: bool = True
    global_linear_pool: bool = True
    paired: bool = True
    vec_mixture: bool = True  # TODO
    gain: bool = True
    pre_gated: bool = True  # TODO
    compressed: bool = True  # TODO
    bias: bool = False
    ...
    separated_aux_loss: bool = False
    expert_token_aux_loss: bool = True
...
class ParamVectorPool(nn.Module):
            ...
            torch.cat(
                [
                    torch.randint(
                        ...
                        config.global_linear_block_num
                        if config.global_linear_pool
                        else config.linear_block_num[
                            config.linear_module_parent_list[i]
                        ],
                        (
                            1,
                            config.linear_cluster_num,
    ...
    def vec_init(self):
        ...
        linear_k_means = KMeans(n_clusters=n_lc, verbose=0)
        ffn_k_means = KMeans(n_clusters=n_fc, verbose=0)
        ...
        # ffn_vec_c_norm = torch.linalg.vector_norm(ffn_vec_centroids, dim=-1)
        # print(f"ffn_vec_c_norm: \n{ffn_vec_c_norm[0].reshape(-1, 8)}")
    ...
    def update(
        ...
        if not init:
            ...
            linear_vec_scores = torch.bmm(
                linear_vec[:, :, : self.config.vec_dim].detach(), linear_vec_c
            )
            ...
            # [n_layer, v, n_fc]
            # ffn_vec_c_norm = torch.linalg.vector_norm(ffn_vec_c, dim=1)
            # print(f"ffn_vec_c_norm: \n{ffn_vec_c_norm[0].reshape(-1, 8)}")

            ffn_vec_scores = torch.bmm(
                ffn_vec[:, :, : self.config.vec_dim].detach(), ffn_vec_c
            )
        ...
        # [n_layer * n_lm, n_lc * n_v]
        # linear_vec_indices_bin = torch.bincount(linear_vec_new_labels.flatten().to(torch.int64))
        # print(f"linear_vec_indices_bin: \n{linear_vec_indices_bin[:n_lc].reshape(-1, 8)}")
        ...
        # [n_layer, 1]
        ffn_vec_new_labels = ffn_vec_labels + ffn_label_offset
        # [n_layer, n_fc * n_v]
        # ffn_vec_indices_bin = torch.bincount(ffn_vec_new_labels.flatten().to(torch.int64))
        # print(f"ffn_vec_indices_bin: \n{ffn_vec_indices_bin[:n_fc].reshape(-1, 8)}")
        ffn_vec_new_labels += self.linear_c_num
        ...
        if not init:
            if self.config.separated_aux_loss:
                vec_probs = torch.cat(
                    (
                ...
                vec_probs = torch.cat(
                    (
                        torch.sigmoid(linear_vec_values).flatten(),
                        torch.sigmoid(ffn_vec_values).flatten()
                        * self.config.update_ffn_balance_ratio,
                    ),
                    0,
                )
            vec_probs_bin = weighted_bincount(vec_labels, vec_probs, vec_c_num)
            # [vec_c_num]

            if self.config.separated_aux_loss:
                linear_c_vec_aux_loss = (
                    torch.dot(
                        vec_indices_bin[: self.linear_c_num].type_as(vec_probs_bin),
                        vec_probs_bin[: self.linear_c_num],
                    )
                    / (
                        linear_vec_num_valid
                        * torch.sum(vec_probs_bin[: self.linear_c_num])
                    )
                    * self.linear_c_num
                )
                ffn_c_vec_aux_loss = (
                ...
                c_vec_aux_loss = (
                    (
                        linear_c_vec_aux_loss
                        + ffn_c_vec_aux_loss * self.config.update_ffn_balance_ratio_sep
                    )
                    / (1 + self.config.update_ffn_balance_ratio_sep)
            ...
            vec_indices_bin_mask = vec_indices_bin != 0
            vec_indices_bin_zero_num = torch.sum(~vec_indices_bin_mask).item()

            if vec_indices_bin_zero_num > 0:
                vec_indices_bin_non_zero = vec_indices_bin[vec_indices_bin_mask]
                vec_probs_bin_non_zero = vec_probs_bin[vec_indices_bin_mask]
                c_mean_prob = torch.mean(
                    vec_probs_bin_non_zero / vec_indices_bin_non_zero
                ).item()
            else:
                c_mean_prob = torch.mean(vec_probs_bin / vec_indices_bin).item()
...
class Query(nn.Module):
    ...
    def forward(
        ...
        if self.config.query_bias:
            ...
            query_ffn = x_ffn_blockwise + x_ffn_fusion
        # [s, n_fr, v]
        if external is not None:
...
class GroupedGEMM(Function):
    ...
    def forward(
        ...
        # for ffn:
        # input: [s, 1, n_fr, 2 * d_f], weight: [m, 3, d_f, d_fh], bias: [m, 1, d_f] or None (m = n_fb), probs: [s, 1, n_fr, k * n_iv, 1]
        ...
        if ffn:
            ...
            del input_1, input_1_g, input_1_l
            ...
            del input_2, input_2_g, input_2_l
        ...
        output_original = torch.empty(
            new_indices.shape[0], output_dim, dtype=dtype, device=device
        )
        # [n + n_p, d_o (d_f)]

        output_original[order_indices_1] = output_1
        output_original[order_indices_2] = output_2
        output_original[order_indices_3] = output_3
        del output_1, output_2, output_3

        # TODO: Triton
        output = output_original[:n]
        # [n, d_o (d_f)]
        del output_original
        if not router:
            output = rearrange(
            ...
            if fused:
                ...
                output = torch.sum(output * probs, reduce_pos, dtype=dtype)
    ...
    def backward(
        ...
        if not router:
            if fused:
                ...
                grad_output_repeat = grad_output.flatten(0, -2)
        else:
            grad_output_repeat = grad_output
        grad_o_zone[ranks_n] = grad_output_repeat
        del grad_output_repeat
        ...
        if ffn:
            ...
            del i_sorted_1, i_sorted_2, i_sorted_3
        ...
        if fused:
            output_1 = (
                torch.bmm(output_1_o, weight_1[:, 2, ...].transpose(1, 2))
                if ffn
                else torch.bmm(i_sorted_1, weight_1)
            )
            # [s_1, f_max, d_o (d_f)]
            if bias is not None:
                output_1 += bias_1
            ...
            output_2 = (
                torch.bmm(output_2_o, weight_2[:, 2, ...].transpose(1, 2))
                if ffn
                else torch.bmm(i_sorted_2, weight_2)
            )
            # [s_2, f_x, d_o (d_f)]
            if bias is not None:
                output_2 += bias_2
            ...
            output_3 = (
                torch.bmm(output_3_o, weight_3[:, 2, ...].transpose(1, 2))
                if ffn
                else torch.bmm(i_sorted_3, weight_3)
            )
            # [s_3, f_y, d_o (d_f)]
            if bias is not None:
                output_3 += bias_3
        ...
        if ffn:
            ...
            # [s_1, f_max, d_fh]
            grad_output_1_l = grad_output_1_o * F.silu(output_1_g)
            # [s_1, f_max, d_fh]
            del grad_output_1_o, output_1_l

            grad_weight_1_o = torch.bmm(
                output_1_o.transpose(1, 2), grad_o_sorted_1
            ).transpose(1, 2)
            del grad_o_sorted_1, output_1_o
            # [s_1, d_fh, f_max] bmm [s_1, f_max, d_f] = [s_1, d_fh, d_f]
            # transpose(1, 2) -> [s_1, d_f, d_fh]

            grad_weight_1_l = torch.bmm(i_sorted_1_l.transpose(1, 2), grad_output_1_l)
            del i_sorted_1_l
            ...
            # [s_1, f_max, d_fh]
            del output_1_g, output_1_g_s

            grad_weight_1_g = torch.bmm(i_sorted_1_g.transpose(1, 2), grad_output_1_g)
            del i_sorted_1_g
            # [s_1, d_f, f_max] bmm [s_1, f_max, d_fh] = [s_1, d_f, d_fh]
            ...
            # [s_2, f_x, d_fh]
            grad_output_2_l = grad_output_2_o * F.silu(output_2_g)
            # [s_2, f_x, d_fh]
            del grad_output_2_o, output_2_l

            grad_weight_2_o = torch.bmm(
                output_2_o.transpose(1, 2), grad_o_sorted_2
            ).transpose(1, 2)
            del grad_o_sorted_2, output_2_o
            # [s_2, d_fh, f_x] bmm [s_2, f_x, d_f] = [s_2, d_fh, d_f]
            # transpose(1, 2) -> [s_2, d_f, d_fh]

            grad_weight_2_l = torch.bmm(i_sorted_2_l.transpose(1, 2), grad_output_2_l)
            del i_sorted_2_l
            ...
            # [s_2, f_x, d_fh]
            del output_2_g, output_2_g_s

            grad_weight_2_g = torch.bmm(i_sorted_2_g.transpose(1, 2), grad_output_2_g)
            del i_sorted_2_g
            # [s_2, d_f, f_x] bmm [s_2, f_x, d_fh] = [s_2, d_f, d_fh]
            ...
            # [s_3, f_y, d_fh]
            grad_output_3_l = grad_output_3_o * F.silu(output_3_g)
            # [s_3, f_y, d_fh]
            del grad_output_3_o, output_3_l

            grad_weight_3_o = torch.bmm(
                output_3_o.transpose(1, 2), grad_o_sorted_3
            ).transpose(1, 2)
            del grad_o_sorted_3, output_3_o
            # [s_3, d_fh, f_y] bmm [s_3, f_y, d_f] = [s_3, d_fh, d_f]
            # transpose(1, 2) -> [s_3, d_f, d_fh]

            grad_weight_3_l = torch.bmm(i_sorted_3_l.transpose(1, 2), grad_output_3_l)
            del i_sorted_3_l
            ...
            # [s_3, f_y, d_fh]
            del output_3_g, output_3_g_s

            grad_weight_3_g = torch.bmm(i_sorted_3_g.transpose(1, 2), grad_output_3_g)
            del i_sorted_3_g
            # [s_3, d_f, f_y] bmm [s_3, f_y, d_fh] = [s_3, d_f, d_fh]
            ...
            # [m, 3, d_f, d_fh]
            grad_weight[freq_sorted_indices_1, 0, ...] = grad_weight_1_g
            grad_weight[freq_sorted_indices_1, 1, ...] = grad_weight_1_l
            grad_weight[freq_sorted_indices_1, 2, ...] = grad_weight_1_o
            grad_weight[freq_sorted_indices_2, 0, ...] = grad_weight_2_g
            grad_weight[freq_sorted_indices_2, 1, ...] = grad_weight_2_l
            grad_weight[freq_sorted_indices_2, 2, ...] = grad_weight_2_o
            grad_weight[freq_sorted_indices_3, 0, ...] = grad_weight_3_g
            grad_weight[freq_sorted_indices_3, 1, ...] = grad_weight_3_l
            grad_weight[freq_sorted_indices_3, 2, ...] = grad_weight_3_o
            ...
            del grad_o_sorted_1, i_sorted_1
            ...
            del grad_o_sorted_2, i_sorted_2
            ...
            del grad_o_sorted_3, i_sorted_3
            ...
            grad_input = grad_input[:n]
            # [n, d_i]
            del grad_input_1, grad_input_2, grad_input_3
...
class Router(nn.Module):
    ...
    def forward(
        ...
        if self.config.router_detach:
            query_linear = query_linear.detach()
            query_ffn = query_ffn.detach()
        ...
        if self.config.gain:
            ...
            ffn_vec_gain = ffn_gain_factors_pool[
                ffn_cluster_indices, ffn_vec_indices, :
            ]
            # [s, 1, n_fr, k, n_iv]
        ...
        # [s * n_lm * n_lr]
        n_iv = 2 if self.config.paired else 1

        linear_vec_probs_aux = torch.sigmoid(linear_vec_values)
        # [s, n_lm, n_lr, 1]
        linear_vec_probs = (
            linear_vec_probs_aux * torch.sigmoid(linear_vec_gain) * 2
            if self.config.gain
            else linear_vec_probs_aux.expand(-1, -1, -1, n_iv)
        )
        # [s, n_lm, n_lr, n_iv]
        ...
        # [s, 1, n_fr, k, 1]
        ffn_vec_probs = (
            ffn_vec_probs_aux * torch.sigmoid(ffn_vec_gain) * 2
            if self.config.gain
            else ffn_vec_probs_aux.expand(-1, -1, -1, -1, n_iv)
        )
        # [s, 1, n_fr, k, n_iv]

        if self.config.separated_aux_loss:
            c_probs_aux = torch.cat((linear_c_probs_aux, ffn_c_probs_aux), 0)
        else:
            c_probs_aux = torch.cat(
                (linear_c_probs_aux, ffn_c_probs_aux * self.config.c_ffn_balance_ratio),
                0,
            )
        # [s * n_lm * n_lr + s * n_fr]
        with torch.no_grad():
        ...
        if self.config.separated_aux_loss:
            linear_c_aux_loss = (
                torch.dot(
                    c_indices_bincount[:num_linear_c_bins].type_as(c_probs_bincount),
                    c_probs_bincount[:num_linear_c_bins],
                )
                / (
                    linear_cluster_indices_flatten.numel()
                    * torch.sum(c_probs_bincount[:num_linear_c_bins])
                )
                * num_linear_c_bins
            )
            ffn_c_aux_loss = (
                ...
                / (
                    ffn_cluster_indices_flatten.numel()
                    * torch.sum(c_probs_bincount[num_linear_c_bins:])
                )
                * num_ffn_c_bins
            )
            c_aux_loss = (
                linear_c_aux_loss + ffn_c_aux_loss * self.config.c_ffn_balance_ratio_sep
            ) / (1 + self.config.c_ffn_balance_ratio_sep)
        else:
            c_aux_loss = (
                torch.dot(
        ...
        block_num = self.config.global_linear_block_num + self.config.ffn_block_num

        if self.config.expert_token_aux_loss:
            expert_indices_aux = torch.cat(
                (
                    linear_block_indices.flatten(),
                    ffn_block_indices.flatten() + self.config.global_linear_block_num,
                ),
                0,
            )
            # [s * n_lm * n_lr * n_iv + s * n_fr * k * n_iv]
            n_iv = 2 if self.config.paired else 1
            expert_probs_aux = vec_probs_aux.unsqueeze(-1).expand(-1, n_iv).flatten()

            with torch.no_grad():
                expert_indices_bincount = torch.bincount(
                    expert_indices_aux, minlength=block_num
                )
            expert_probs_bincount = weighted_bincount(
                expert_indices_aux, expert_probs_aux, block_num
            )
        else:
            ...
            # [s * n_fr * k]
            vec_indices_aux = torch.cat(
                (linear_vec_indices_aux, ffn_vec_indices_aux), 0
            )
            # [s * n_lm * n_lr + s * n_fr * k]
        ...
        if self.config.separated_aux_loss:
            linear_vec_aux_loss = (
                torch.dot(
                    vec_indices_bincount[:num_linear_vec_bins].type_as(
                        vec_probs_bincount
                    ),
                    vec_probs_bincount[:num_linear_vec_bins],
                )
                / (
                    linear_cluster_indices_flatten.numel()
                    * torch.sum(vec_probs_bincount[:num_linear_vec_bins])
                )
                * num_linear_vec_bins
            )
            ffn_vec_aux_loss = (
            ...
            vec_aux_loss = (
                linear_vec_aux_loss
                + ffn_vec_aux_loss * self.config.vec_ffn_balance_ratio_sep
            ) / (1 + self.config.vec_ffn_balance_ratio_sep)
        else:
            if self.config.expert_token_aux_loss:
                vec_aux_loss = (
                    torch.dot(
                        expert_indices_bincount.type_as(expert_probs_bincount),
                        expert_probs_bincount,
                    )
                    / (expert_indices_aux.numel() * torch.sum(expert_probs_bincount))
                    * block_num
                )
            else:
                vec_aux_loss = (
...
class RotaryEmb(nn.Module):
    def __init__(self, config: Config):
        ...
        self.config = config
        freqs = (1 / config.theta) ** (torch.arange(0, config.attn_block_dim, 2) / config.attn_block_dim)
        # [d_a / 2]
...
class Layer(nn.Module):
    ...
    def layer_router_info(self, router_results, sublayer):
        ...
        if self.config.global_linear_pool and sublayer != "ffn":
...
for step in range(num_steps + 1):
    ...
    if config.scaler:
        scaler.scale(c_vec_aux_loss).backward()
    else:
        c_vec_aux_loss.backward()
    ...
    if master_process:
        ...
        residual_coeff_avg = (
            sum(residual_coeff_list) / len(residual_coeff_list)
            if len(residual_coeff_list) > 0
            else 0
        )
        residual_norm_coeff_avg = (
            sum(residual_norm_coeff_list) / len(residual_norm_coeff_list)
            if len(residual_norm_coeff_list) > 0
            else 0
        )
        ...
        c_vec_aux_loss = c_vec_aux_loss.item()
